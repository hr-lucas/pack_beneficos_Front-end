<template>
    <div>
        ...
        <div class="max-w-xl mx-auto space-y-2 text-center lg:max-w-none">
            <div class="relative z-0 inline-flex overflow-hidden bg-white border-2 p-2 border-secondary  rounded-full">
                <button class="block w-full px-8 py-2 text-sm font-medium transition rounded-full " :class="annual == true ? ' bg-secondary !text-white' : ''" @click="annual = true" type="button">
                  Anual
                </button>
              <button class="block w-full px-8 py-2 text-sm font-medium  transition rounded-full " :class="annual == false ? 'bg-secondary text-white' : ''" @click="annual = false" type="button">
                Mensal
              </button>
            </div>
          </div>
      ...
      >
        <div
        v-for="(plan, index) in selectPlan" :key="index"
        ...
        >
          ...
          <div>
            ...
            ><span class="text-gray text-[12px]"
              >*Pagamento Cartão de Crédito.</span
            >
            <div class="w-full">
              <ul class="w-full flex flex-col items-start">
                <li
                v-for="(details, index) in plan.details" :key="index"
    ...
    anual: [
        {
            name:'Plano Básico',
            price: 'R$ 24,90',
            description: 'Ideial para quem precisa de orientação médica básica e regularmente.',
            details: ['Teleconsulta', 'Adicional de R$ 3,49/mês por dependente'],
        },
        {
            name:'Plano Especialistas',
            price: 'R$ 49,90',
            description: 'Perfeito para quem deseja um cuidado completo, incluindo saúde mental, física e nutricional.',
            details: ['Teleconsulta','Pronto atendimento','Nutrição','Psicologia', 'Adicional R$ 9,90/mês por dependente'],
        }
    ],
    mensal: [
        {
        ...
        },
        {
            ...
            price: 'R$ 64,90',
            description: 'Perfeito para quem deseja um cuidado completo, incluindo saúde mental, física e nutricional.',
            details: ['Teleconsulta','Pronto atendimento','Nutrição','Psicologia', 'Adicional R$ 14,90 por dependente'],
        }
    ],
}
</script>
